...
import { useCreateRewardService, useUpdateRewardService } from "@/services/spin.service";
...
export function SpinRewardForm({
    ...
    const forbiddenThemeIds = useMemo(() => {
        ...
        if (initialData) {
            ...
            prevReward =
                existingRewards.length > 1
                    ? existingRewards[(index - 1 + existingRewards.length) % existingRewards.length]
                    : undefined;
            nextReward =
                existingRewards.length > 1
        ...
        return SPIN_REWARD_THEMES.filter((t) =>
            forbiddenColors.has(t.color)
        ).map((t) => t.id);
    ...
    const onSubmit = async (values: SpinFormValues) => {
        ...
        if (initialData) {
            updateReward(initialData.id, payload);
        } else {
                                    ...
                                    >
                                        ...
                                        <SelectContent>
                                            {SPIN_REWARD_THEMES.map((theme) => {
                                                const isForbidden = forbiddenThemeIds.includes(theme.id);
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            />
                                                            <span className={isForbidden && !isCurrent ? "text-muted-foreground" : "text-foreground"}>
                                                                {theme.label} {isForbidden && !isCurrent ? "(Clash)" : ""}
                                                            </span>
                                                        </div>
                                                    </SelectItem>
                                                );
